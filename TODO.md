...
- Change these to install under `/usr/lib`: `/usr/opt/yaml_cpp_vendor/lib`, `/usr/opt/rviz_ogre_vendor/lib`.
  (`WIP yaml-cpp-vendor.inc: install yaml-cpp files`.)
...
- Move PV of kernel, others(?) to distro.
...
- Update bouncy, crystal ros-recipe-blacklist.inc from melodic. php-native??
- Add P_V for imported recipes that have been upgraded: cppunit python-matplotlib => add common ros-preferred-versions.inc
...
- pcl.inc: PARALLEL_MAKE = "-j TRUNC(getmem/5)"
...
- Do the recipes for the components found in `files/<ROS_DISTRO>/newer-system-components.txt` need
 `inherit ros_distro_${ROS_DISTRO}` ?
- r26: recipes-{ros1,ros2} -> includes-{ros1,ros2}; move "ROS infrastructure" packages from recipes-* to recipes-ros instead of
  recipes-ros-extra?
...
- `WORKAROUND ros2.bbclass: Replace hardcoded sysroot in generated *.cmake files`: sed-ing is in ros2.bbclass. Is this correct?
...
- Re-order commits along`[r.hk/thud-superflore-crystal]`:
    ...
    - Update versions of system package recipes to Ubuntu 18.04 ones; import several from other layers.
