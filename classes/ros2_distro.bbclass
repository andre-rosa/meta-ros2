...
do_install_append() {
    ...
    if [ -n "${RECIPE_SYSROOT}" ]; then
        find ${D} -name '*.cmake' | xargs sed -i -e 's@${RECIPE_SYSROOT}@${STAGING_DIR_HOST}@g'
    else
        true
    fi
}
...
do_install_append_class-target() {
    # The parent_prefix_path files are used on the target => remove the build tree from the paths. We assume this is only an issue with ROS 2.
    if [ -d ${D}${datadir}/ament_index/resource_index/parent_prefix_path ]; then
        find ${D}${datadir}/ament_index/resource_index/parent_prefix_path -type f | xargs sed -i -e 's@${STAGING_DIR_HOST}@@g' -e 's@${STAGING_DIR_NATIVE}@@g'
    else
        true
    fi
}
