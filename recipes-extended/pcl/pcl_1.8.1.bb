...
# Do one compile at a time by default, just to be safe (even though it will take nearly 3 hours). However, using OE thud,
# quick experiments have shown that it's possible to use up to ROUND(<RAM-in-GB>/5) as the argument to -j without running out of
# memory as long as the machine has a few GB of swap space. Override the default by setting PARALLEL_MAKE_pn-pcl = "-j <N>" in
# conf/local.conf .
PARALLEL_MAKE = "-j 1"
